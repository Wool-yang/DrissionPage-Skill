...
当需要生成 `.dp/tmp/` 临时脚本，或把已验证流程沉淀为
`.dp/projects/<site>/scripts/*.py` 脚本时，读取本文。

本文提供截图、抓取、登录、表单、上传、下载、新标签页、WebPage 和 SessionPage
等 action templates / execution primitives。浏览器/WebPage 模板使用统一连接头；
纯请求 `SessionPage` 模板使用独立脚本头，不接管浏览器。
Action templates / execution primitives 是页面动作模板；站点 workflow 建模由 discovery reference 承载。

站点 workflow discovery 是独立的站点流程建模层。
当页面结构、候选选择器、复用路径或输出契约不清楚时，先读
...
- [Template 1：截图 screenshot](#template-1截图screenshot)
- [Template 2：数据抓取 scrape](#template-2数据抓取scrape)
- [Template 3：自动登录 login](#template-3自动登录login)
- [Template 4：表单填写 form](#template-4表单填写form)
- [Template 5：文件上传 upload](#template-5文件上传upload)
- [Template 6：文件下载 download](#template-6文件下载download)
- [Template 7：新标签页 new-tab](#template-7新标签页new-tab)
- [Template 8：WebPage cookie 同步 web-page-sync](#template-8webpage-cookie-同步web-page-sync)
- [Template 9：自定义多步任务 custom](#template-9自定义多步任务custom)
- [Template 10：SessionPage 纯请求 session-page](#template-10sessionpage-纯请求session-page)
...
- 纯请求 `SessionPage` 任务复制 Template 10 的独立脚本头，不导入 provider 连接 helper
- 临时任务写到 `.dp/tmp/`；简单一次性任务可以覆盖 `_run.py`，多轮探索或调试使用语义化文件名
- 值得沉淀的任务写到 `.dp/projects/<site>/scripts/<name>.py`
- 独立脚本默认通过 `site_run_dir(site, script_name)` 创建 run-dir
- 外层编排传入 `run_dir` 时，子步骤复用该目录，不再创建新的 run-dir
...
但连接函数换成 `start_profile_and_connect_web_page()`。纯请求 `SessionPage` 见 Template 10。

```python
"""
site: <site-name>
workflow_summary: <workflow 的一句话摘要>
...
## Template 8：WebPage cookie 同步（web-page-sync）
...
| 优先级 | 匹配条件 | 置信度 | 推荐操作 |
|---|---|---|---|
| 1 | site + intent + status: ok，且入口 / 输出契约兼容 | 高 | 直接复用，无需询问 |
| 2 | url / 入口状态 / 输出契约部分匹配 | 中 | 读取脚本后决定复用、微调或修复 |
| 3 | 只有 workflow_summary 文本相近，入口、状态或输出契约不明确 | 低 | 先读旧脚本验证；页面结构或复用路径仍不清楚时做 Workflow Discovery |

**边界原则**：

- `workflow_summary` 是 workflow 的人类可读摘要，不是稳定 ID，也不是精确复用键
- `site + intent` 需要结合 `status: ok`、入口状态和输出契约判断，匹配时直接静默复用
- `url`、入口状态和输出契约区分同站点不同子场景，匹配时读脚本后再判断
- 只有 `workflow_summary` 摘要文本相近时，先读旧脚本验证；仍低置信时按
  `references/workflow-discovery.md` 建立 workflow 模型，再决定复用、修复或新建
...
# 场景 C：workflow_summary 文本相近但入口或输出契约不精确 → 先 Workflow Discovery，验证后再沉淀
```
