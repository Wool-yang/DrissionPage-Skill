...
复杂导出先做 no-submit（不提交）探测。探测阶段可以展开二级菜单、打开弹窗、查看 range modal、
读取默认范围和可选项，但不要点击最终提交/生成/发送按钮；除非用户已经明确要求执行导出，
否则 discovery 只记录触发链路和状态判断。

探测 range modal 或导出弹窗时，提交前必须做 readback：

- 记录日期范围、筛选条件、文件格式、导出粒度和收件人/目标位置
- 保存 `range-readback.json`，包含字段选择器、期望值、实际值、可见文本和选中态
- 对关键字段做断言；readback 不一致时停止，不进入真实导出

导出探测至少记录：

- 触发元素候选选择器
- 是否需要二级菜单、弹窗、range modal、新 tab 或后台任务
- 文件名能否预测，是否需要语义重命名
- 完成状态如何判断
- no-submit 阶段是否可完整读回导出条件
- `export-probe.json`：触发链路、候选选择器、弹窗/range 状态、后台任务入口和风险点
- `range-readback.json`：如果存在范围或导出条件表单，记录提交前 readback 证据
